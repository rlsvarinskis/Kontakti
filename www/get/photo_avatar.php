...
if (isset($_GET['uid']))
{
	if ($stmt = $db->prepare("SELECT `mediaid`,`type` FROM `accounts` JOIN `media` ON `accounts`.`picture`=`media`.`mediaid` WHERE `accounts`.`id`=? AND `media`.`deleted`=0"))
	{
...
} else
{
	...
	{
		$stmt->bind_param("s", $userdata['picture']);
		$stmt->execute();
		$result = $stmt->get_result();
		if (mysqli_num_rows($result) > 0)
		{
