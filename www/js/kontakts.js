var kontakts = function(options)
{
	this.options = {
		...
		type: (!options || typeof options.type !== "number") ? NaN : options.type,
		title: (!options || typeof options.title !== "number") ? "Kontakts" : options.title
	}
	
	this.kontakts = []; //The list of all kontakts
	
	kontakts_createArrayElement(this.kontakts, this.options.title);
...
kontakts.prototype.displayNewKontakts = function(thelist)
{
	for (var i = 0; i < thelist.length; i++)
	{
		...
		kontakt.list = thelist; //TODO memory leak
		if (!kontakt.element)
		{
